...
- **`ghsas/all.json`** (DEC-ARCHIVE-011): GitHub advisory IDs matched with GitHub's own alphabet (`[23456789cfghjmpqrvwx]`, so prose never false-matches), stored as `GHSA-` + lowercase, from subject + body. Unsharded because GHSA IDs have no year (~600 today); keyed `all` in the manifest so every section keeps the `{shard: {count, sha256}}` shape. Indexed as written: repo-level and draft advisories included, no GHSA↔CVE aliasing (that's the consumer's job, e.g. via OSV). `verify-index.pl` cross-checks it against every row's `ghsa_ids` in both directions. Adding a per-row field rewrites every `messages/` shard; that was accepted once for `ghsa_ids` — think again before doing it routinely.
